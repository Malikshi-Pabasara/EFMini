...
namespace Cenium.Contacts.Activities
{
    ...
    public class ContactsActivity : IDisposable
    {
        ...
        {
            return _ctx.Contacts.ReadOnlyQuery().FirstOrDefault(p => p.ContactId == contactId);
        }
        ...
        private void ValidateNameExists(Contact contact)
        {
            var result = contact.ContactId != 0 ?
                _ctx.Contacts.ReadOnlyQuery().FirstOrDefault(i => i.Name == contact.Name && i.ContactId != contact.ContactId) :
                _ctx.Contacts.ReadOnlyQuery().FirstOrDefault(i => i.Name == contact.Name);
            if (result != null)
            {
